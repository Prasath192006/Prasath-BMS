<img src="https://capsule-render.vercel.app/api?type=waving&height=250&color=0:0D1117,100:38BDAE&text=Prasath%20B%20M%20S&fontColor=FFFFFF&fontSize=50&animation=twinkling&fontAlignY=38&desc=CS%20Student%20-%20Full%20Stack%20Developer%20-%20ML%20Enthusiast&descAlignY=58&descSize=22" width="100%"/>

<div align="center">

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=28&duration=3000&pause=1000&color=38BDAE&center=true&vCenter=true&width=800&lines=Hi%2C+I'm+Prasath+B+M+S+%F0%9F%91%8B;Computer+Science+and+Engineering+Student;Full+Stack+Developer+%F0%9F%9A%80;MERN+Stack+Enthusiast+%F0%9F%92%BB;Machine+Learning+Explorer+%F0%9F%A4%96" alt="Typing SVG" />
</p>

</div>

---

## 🧑‍💻 About Me

```java
public class Developer {

    private final String name     = "Prasath B M S";
    private final String location = "Salem, Tamil Nadu, India";
    private final String degree   = "B.E. Computer Science and Engineering @ Karpagam College of Engineering";
    private final String cgpa     = "8.49 / 10.0";
    private final String email    = "prasath7102@gmail.com";
    private final String phone    = "+91 7010840702";

    private final String[] techStack = {
        "C++", "Java", "JavaScript", "Python (Basics)",
        "HTML", "CSS", "React.js", "Material-UI",
        "Node.js", "Express.js",
        "MySQL", "MongoDB"
    };

    private final String[] currentlyLearning = {
        "Spring Boot",
        "Advanced Backend Architecture"
    };

    private final String[] interests = {
        "Full Stack Development",
        "Machine Learning",
        "Agri-Tech Innovation",
        "Hackathons & Rapid Prototyping"
    };

    private final String funFact =
        "I enjoy turning real-world problems—like connecting farmers to markets or summarizing videos—into working full-stack apps 🌱";

    public String motto() {
        return "Build with purpose. Code with precision. Ship with confidence.";
    }
}
```

---

## 🛠️ Tech Stack

### 💻 Languages

<p>
  <img src="https://skillicons.dev/icons?i=cpp,java,js,python,html,css" />
</p>

### 🚀 Frameworks & Libraries

<p>
  <img src="https://skillicons.dev/icons?i=react,nodejs,express,materialui" />
</p>

### 🗄️ Databases

<p>
  <img src="https://skillicons.dev/icons?i=mysql,mongodb" />
</p>

### 🛠️ Tools & Platforms

<p>
  <img src="https://skillicons.dev/icons?i=vscode,idea,docker,git,github,postman,eclipse" />
</p>

---

## 📊 GitHub Stats

<div align="center">

<img src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=Prasath192006&theme=tokyonight" />

</div>

---

## 💻 Top Languages

<div align="center">

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Prasath192006&layout=compact&theme=tokyonight&hide_border=true&count_private=true" />

</div>

---

## 🔥 Streak Stats

<div align="center">

<img src="https://streak-stats.demolab.com?user=Prasath192006&theme=tokyonight&hide_border=true&mode=weekly" />

</div>

---

## 📈 Activity Graph

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=Prasath192006&theme=tokyo-night&bg_color=0d1117&color=38bdae&line=a855f7&point=bf91f3&area=true&area_color=38bdae&hide_border=true&height=300&custom_title=Prasath's%20Contribution%20Graph" width="100%"/>

</div>

---

## 🚀 Featured Projects

<div align="center">

| Project | Stack | Highlights |
|:---|:---|:---|
| [🌾 AgriConnect Platform](https://github.com/Prasath192006/AgriConnect_Frontend) • [Backend](https://github.com/Prasath192006/AgriConnect_Backend) | `Spring Boot` `React` `MySQL` | Digital agriculture platform connecting farmers and buyers directly to improve price transparency and eliminate middlemen. Features a scalable Spring Boot backend with REST APIs, authentication, role-based access control, crop listing and order workflows, along with a responsive React frontend for crop discovery, seller profiles, and real-time availability. |
| [**🎬 Movie Review Success Predictor**](https://github.com/Prasath192006/Movie_Review_Success_Predictor) | `Python` `NLP` `TF-IDF` `Linear SVM` | Personal ML project that classifies movie success (Hit/Flop) using the IMDb 50,000-review dataset — applies TF-IDF vectorization for feature extraction and a Linear SVM model achieving over 90% accuracy |
| [**👥 Employee Management System**](https://github.com/Prasath192006/TeamFlow_EmployeeManagement) | `MERN Stack` `Node.js` `Express` `MongoDB` `Material-UI` | Full-stack employee and task management system with role-based authentication for managers and employees — 15+ RESTful APIs supporting complete CRUD operations and secure profile image uploads, deployed on Netlify and Render |
| [**📺 YouTube Video Summarizer**](https://github.com/Prasath192006/YouTube_video_Summarizer_AI_HACKATHON) | `Chrome Extension` `Gemini API` | Built in a 3-day hackathon — a Chrome extension that summarizes YouTube videos using the Gemini API and the youtube-transcript-api, with multi-language support, persistent chat history, and real-time Q&A on video content |

</div>

---

## 🏅 Achievements & Certifications

* 🎓 **CGPA 8.49 / 10.0** – B.E. Computer Science and Engineering
* 🟢 **React Advanced** – Coursera (Meta)
* 🟢 **React Basics** – Coursera (Meta)
* 🟠 **CSS** – HackerRank
* 🟠 **Node.js** – HackerRank
* 🟠 **JavaScript** – HackerRank
* 🟠 **SQL** – HackerRank

---

## 🎓 Education

<div align="center">

| Degree | Institution | Period | Score |
|:---|:---|:---:|:---:|
| 🎓 B.E. Computer Science and Engineering | Karpagam College of Engineering,Coimbatore | Present – Expected May 2027 | **8.49 / 10.0 CGPA** |
| 📘 Higher Secondary (Computer Science) | AKV Matric Hr. Sec. School,Salem | 2022-2023 | 95% |

</div>

---

## 🌱 Currently Learning

```
🍃 Spring Boot           →  Backend Architecture, REST APIs
🌐 Full Stack Dev        →  MERN Stack, Spring Boot, MySQL
🤖 Machine Learning      →  NLP, Text Classification
```

---

## 💪 Soft Skills

```
🤝 Team Collaboration     |  🧩 Problem Solving
⚡ Rapid Prototyping      |  🎯 Attention to Detail
```

---

<div align="center">

### 📬 Connect with Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/prasath-bms-153947241)
[![LeetCode](https://img.shields.io/badge/LeetCode-FFA116?style=flat-square&logo=leetcode&logoColor=white)](https://leetcode.com/u/prasath_006/)
[![Email](https://img.shields.io/badge/Email-EA4335?style=flat-square&logo=gmail&logoColor=white)](mailto:prasath7102@gmail.com)

*"Build with purpose. Code with precision. Ship with confidence."*

</div>

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12,20,24&height=120&section=footer&animation=twinkling" width="100%"/>
